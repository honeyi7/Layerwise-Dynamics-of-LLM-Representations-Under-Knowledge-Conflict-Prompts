...
jobname=check_logits_qwen2.5_0.5B_$(date +%Y%m%d_%H%M%S)

NNODES=1
GPUS_PER_NODE=1

SCRIPT=main/check_logits/code/check_logits.py

SPEC=N6lS.Iu.I80.${GPUS_PER_NODE}

MODEL_PATH=/path/to/Qwen2.5-0.5B-Instruct
KNOW_DATA=main/knowledge_probe/result/qwen2.5_0.5B/known_bucket.jsonl
UNKNOWN_DATA=main/knowledge_probe/result/qwen2.5_0.5B/unknown_bucket.jsonl
AMBIGUOUS_DATA=main/knowledge_probe/result/qwen2.5_0.5B/ambiguous_bucket.jsonl


INTERVENTION_KIND=state 


OUTPUT_DIR=main/check_logits/results/qwen2.5_0.5B
...
INSTALL_DEPS="${PYTHON_CMD} -m pip install accelerate seaborn matplotlib --user"
...
echo "======================================"
echo "Job: ${jobname}"
echo "Model: 0.5B Baseline Check"
